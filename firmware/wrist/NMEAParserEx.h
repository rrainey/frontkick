 ...
 *
 * Author : Glinnes Hulden with extensions by Riley Rainey
 * 
 * This specific file was extracted from the current Arduino version 
 * and included in the Frontkick project with Riley's modifications.
 *
 ...
 */
template <size_t S> class NMEAParserEx {
...
private:
  ...
  class NMEAParserStringify {
    uint8_t       mPos;
    char          mTmp;
    NMEAParserEx<S> *mParent;
  public:
    NMEAParserStringify(NMEAParserEx<S> *inParent, uint8_t inPos) :
      mPos(inPos),
      mParent(inParent)
    {
...
public:
  ...
  bool getArg(uint8_t num, unsigned long &arg)
  {
    if (validArgNum(num)) {
      uint8_t startPos = startArgPos(num);
      uint8_t endPos = endArgPos(num);
      {
        NMEAParserStringify stfy(this, endPos);
        arg = strtoul(&mBuffer[startPos], NULL, 10);
      }
      return true;
    }
    else return false;
